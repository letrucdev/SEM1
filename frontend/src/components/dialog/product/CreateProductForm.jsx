...
export const CreateProductForm = forwardRef(({ onSuccess }, ref) => {
	...
	const handleRemoveImage = (field, src) => {
		const filesValue = event.target.files

		setImagesPreviews(imagesPreviews.filter((url) => url !== src))

		//Undone
		field.onChange(
			field.value.filter((file) => file.name !== src.split('/').pop())
		)
	}
								...
								<div className='grid grid-cols-3 gap-4'>
									{imagesPreviews.map((preview, index) => (
										<ProductPreviewImage
											src={preview}
											key={preview}
											onRemove={() => handleRemoveImage(field, preview)}
										/>
									))}
								</div>
							)}
